...
// 使用：<select class="selectProvince"></select>  <select class="selectCity"></select>
// 重置：$('.selectCity').val('').find("option[value!='']").remove();// 清空“城市”下拉
...
$(document).ready(function(e) {
    ...
    // 初始化 “省份”下拉列表
    var _proArr = [
        '北京市','上海市','天津市','重庆市','河北省','山西省','内蒙古自治区','辽宁省','吉林省','黑龙江省',
        '江苏省','浙江省','安徽省','福建省','江西省','山东省','河南省','湖北省','湖南省','广东省',
        '广西壮族自治区','海南省','四川省','贵州省','云南省','西藏自治区','陕西省','甘肃省','宁夏回族自治区','青海省',
        '新疆维吾尔族自治区','香港特别行政区','澳门特别行政区','台湾省','其它'
    ];

    // 设置 下拉列表 宽度
    $(".selectProvince").css( 'width', '150px' );
    $(".selectCity").css( 'width', '150px' );

    // 设置 下拉列表 内容
    var _content = '<option value="" style="display: none" selected>省份</option><option value="">全部</option>';
    for( var i=0; i < _proArr.length; i++ ){
        _content += '<option value=' + _proArr[i] + '>' + _proArr[i] + '</option>';
    }
    $(".selectProvince").html( _content ); // 省份
    $(".selectCity").html('<option value="" style="display: none" selected>城市</option>'); // 城市
    ...
    $(".selectProvince").change(function(){
        try{
            ...
            // 插入 城市
            $(".selectCity").html('<option value="" style="display: none" selected>城市</option><option value="">全部</option>');
            for( j=0; j < tmpcity.length; j++ ){
                $(".selectCity").append("<option value='" + tmpcity[j] + "'>" + tmpcity[j] + "</option>");
            }
